...
# ===== COLOR PALETTE =====
$TEXT_COLOR    = "White"
$ACCENT_COLOR  = "Cyan"         # Sửa lại thành Cyan thay vì White
$SUCCESS_COLOR = "Green"
$ERROR_COLOR   = "Red"
...
function Show-Loading {
    ...
    $chars = @('|', '/', '-', '\')
    $pos = 0
    $endTime = (Get-Date).AddSeconds(2)  # giả lập 2s
...
function Test-SystemRequirements {
    ...
    if ($ramSpeed) { Write-Host " | Bus: $ramSpeed MHz" -ForegroundColor $TEXT_COLOR } else { Write-Host " | Bus: Không lấy được" -ForegroundColor $ERROR_COLOR }
    ...
    $cFree = ($disks | Where-Object DeviceID -eq 'C:').FreeGB
...
# Show app list với ESC handling đúng
function Show-AppList {
    ...
    foreach ($category in $AppCategories.Keys) {
        ...
        for ($j = 0; $j -lt $half; $j++) {
            ...
            Write-Host $line1 -NoNewline -ForegroundColor $ACCENT_COLOR
            if ($app2) {
                Write-Host "  " -NoNewline
                Write-Host $line2 -ForegroundColor $ACCENT_COLOR
            } else {
                Write-Host ""
            }
        }
...
function Install-AppQuick {
    ...
    foreach ($item in $items) {
        ...
        try {
            if ($app) {
                winget install --id $app.ID --silent --accept-package-agreements --accept-source-agreements --disable-interactivity 2>$null
            } else {
                winget install --id $item --silent --accept-package-agreements --accept-source-agreements --disable-interactivity 2>$null
            }
            Write-Host "`n  ✓ Đã cài: $appName" -ForegroundColor $SUCCESS_COLOR
            $success++
        } catch {
            Write-Host "`n  ✗ Lỗi: $appName" -ForegroundColor $ERROR_COLOR
            $fail++
        }
    }
    
    Write-Host "`n======================" -ForegroundColor $BORDER_COLOR
    Write-Host "Kết quả: $success thành công, $fail thất bại" -ForegroundColor $(if ($fail -eq 0) { $SUCCESS_COLOR } else { $WARNING_COLOR })
    Write-Host ""
...
# Main menu với ESC handling đúng
...
try {
    ...
    Write-Host ""
    Write-Host "Đã xảy ra lỗi: $_" -ForegroundColor $ERROR_COLOR
    Write-Host ""
